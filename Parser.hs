...
ordinal = do
  (match "first" >> return 1)    <|> (match "second" >> return 2) <|>
   (match "third" >> return 3) <|> (match "fourth" >> return 4) <|>
   (match "fifth" >> return 5)  <|> (match "sixth" >> return 6) <|>
   (match "seventh" >> return 7) <|> (match "eighth" >> return 8) <|>
