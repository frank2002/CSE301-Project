...
--------------------------------------------------------------------
-- This function takes an StdGen and returns an infinite list of StdGen values
-- generateStdGenList :: SR.StdGen -> [SR.StdGen]
-- generateStdGenList gen = let (gen1, gen2) = SR.split gen in gen1 : generateStdGenList gen2
...
fightEnemy :: Hero -> Enemy -> SR.StdGen -> IO (Maybe Hero)
fightEnemy hero enemy gen
  | heroIsDead hero = do
      putStrLn "The hero is dead."
      return Nothing
  | enemyIsDead enemy = do
      putStrLn "The enemy is dead."
      return (Just hero)
  | otherwise = do
      ...
      putStrLn $ "Enemy attacks: " ++ show enemyHit
      putStrLn $ "Hero HP: " ++ show (totalHealthPoints heroAfterAttack) ++ ", Enemy HP: " ++ show (healthPoints enemyAfterAttack)
      ...
      else fightEnemy heroAfterAttack enemyAfterAttack gen''

      -- Print attack information
    --   printAttack "Hero" heroHit hero enemy
    --   printAttack "Enemy" enemyHit hero enemy

      -- Recursive call if both are still alive
    --   fightEnemy heroAfterAttack heroAfterAttack gen''
...
testFightEnemy hero enemy gen = do
  finalHeroMaybe <- fightEnemy hero enemy gen
  case finalHeroMaybe of
    Nothing -> putStrLn "The hero is dead."
